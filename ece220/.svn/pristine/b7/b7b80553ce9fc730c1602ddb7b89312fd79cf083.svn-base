...
/*
Introduction
  In this MP, I finish several branches of functions that implement required functionalities.
  1. init_slicing_tree initialize a left-skewed tree with modules generated by function read_modules.
     the tree generated only contains vertical cutlines as the internal nodes in the tree.
  2. is_leaf_node, is_internal_node, is_in_subtree. These three functions are helper functions that help
     me check whether the given node(s) is a leaf (aka module) or a internal node (aka cutline) or one node is in
     subtree rooted at the other node.
  3. get_expression, postfix_traversal. These two functions are order to get the postfix expression of a given tree.
     get_expression function will use postfix_traversal function to tranverse the tree for its left subtree and its
     right subtree and record the node information to an array called "expression"
  4. rotate, recut, swap_module, swap_topology. These four functions are used to change the shape of the tree. Using These
     four functions, we can manipulate the location of each module and find the optimize combination of modules to make the
     size of the chip as small as possible.
*/
...
void swap_topology(node_t* a, node_t* b) {
  ...
  a->parent = node2;
  b->parent = node1;
  // four cases for left or right branches of parent node
