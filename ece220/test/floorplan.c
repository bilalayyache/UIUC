...
int is_leaf_node(node_t* ptr) {
  ...
  if (ptr == NULL)
    return 0;
  if (ptr->module != NULL)
    return 1;                   // if module is not NULL, it is a module
  else
    return 0;
}
...
int is_internal_node(node_t* ptr) {
  ...
  if (ptr == NULL)
    return 0;
  if (ptr->module == NULL)
    return 1;                   // if module is NULL, it is a internal node
  else
    return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  if (a == b)
    return 1;
  // 2. if a is parent of b
  else if (a == b->parent)
    return 1;
  // 3. if b is root of the tree
...
void rotate(node_t* ptr) {
  ...
  if(!is_leaf_node(ptr))                    // if inout node is not leaf
    return;
  int temp;                                 // begin swap width and height
  temp = ptr->module->w;
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if (ptr->cutline == V)                    // swap the type of the cutline
    ptr->cutline = H;
  else
    ptr->cutline = V;
  return;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  // there are four things need to be modified
  // swap parent node of two nodes
  // swap parent's left/right node of two parent nodes of givin nodes

  node_t *node1 = a->parent;
  node_t *node2 = b->parent;
  a->parent = node2;
  b->parent = node1;
  if (node1->left == a && node2->left == b){
    node1->left = b;
    node2->left = a;
  }
  else if (node1->left == a && node2->right == b){
    node1->left = b;
    node2->right = a;
  }
  else if (node1->right == a && node2->left == b){
    node1->right = b;
    node2->left = a;
  }
  else{
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  // for postfix order, we must first go traverse left branch and right branch
  // and at last assign expression
  postfix_traversal(ptr->left, nth, expression);
  postfix_traversal(ptr->right, nth, expression);
  if (ptr->module != NULL)                         // if the node is a module
    expression[*nth].module = ptr->module;
  else                                             // if the node is a internal node
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // create a left-skewed tree with num_modules nodes (n from 0 to (num_modules - 1))
  // all modules are stored in array modules[n]  *nth += 1;                                        // depth of the traverse increase 1
  ...
  // base case
  if (n == num_modules - 1){
    // notice "modules" is the pointer points to modules[0]
    // if we want the pointer to modules[n], we need (modules + n)
    node->module = modules + n;
    node->cutline = UNDEFINED_CUTLINE;
    return node;
  }
  // recursive case
  // we need to create a leaf (right leaf) in this case
  else{
    node_t *right_leaf = (node_t *)malloc(sizeof(node_t));
    // initialize right_leaf
    right_leaf->module = modules + n;
    right_leaf->cutline = UNDEFINED_CUTLINE;
    right_leaf->parent = node;
    right_leaf->left = right_leaf->right = NULL;
    node->right = right_leaf;
    // do recursive case for left subtree
