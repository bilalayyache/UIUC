...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      // get every pixel from the image and calculate the distance from center
      HSLAPixel & pixel = image.getPixel(x, y);
      dis = sqrt((x - centerX)*(x - centerX) + (y - centerY) * (y - centerY));
      // if distance is less than 160, new l is 0.975 previous l
      // else new l is 0.2 previous l
      if (dis <= 160)
        pixel.l *= (1 - 0.005 * dis);
      else
        pixel.l *= 0.2;
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      HSLAPixel const & pixel2 = secondImage.getPixel(x, y);
      if (pixel2.l == 1.0)
