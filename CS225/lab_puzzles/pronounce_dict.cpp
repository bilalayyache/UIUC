...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    map<string, vector<string>>::const_iterator lookup1 = dict.find(w1);
    map<string, vector<string>>::const_iterator lookup2 = dict.find(w2);
    if (lookup1 == dict.end() || lookup2 == dict.end())
      return false;
    else{
      vector<string> s1 = dict.at(w1);
      vector<string> s2 = dict.at(w2);
      if (s1.size() != s2.size())
        return false;
