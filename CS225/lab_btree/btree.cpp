...
void BTree<K, V>::insert(const K& key, const V& value)
{
    ...
    if (root->elements.size() >= order) {
        ...
        // cout << "for the root node, before split:" << endl;
        // cout << *new_root << endl;
        // cout << "new_root child:" << endl;
        // cout << *(new_root->children[0]) << endl;
        split_child(new_root, 0);
        root = new_root;
        // cout << "after split the root, root is:" << endl;
        // cout << *root << endl;
        // cout << "root left is" << endl;
        // cout << *(root->children[0]) << endl;
        // cout << "root right is" << endl;
        // cout << *(root->children[1]) << endl;
    }
}
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // first, we need to insert the middle value of the node to the proper location in parent
    parent->elements.insert(elem_itr, child->elements[mid_elem_idx]);
    // second, insert new_right to the parent children vector
    parent->children.insert(child_itr, new_right);
    // // we need to copy the second half of new_left to new_right
    ...
    //
    // at last, modify the value in child pointer
    for (auto it = mid_elem_itr+1; it != child->elements.end(); it++)
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    // cout << "subroot is:" << endl;
    // cout << *subroot << endl;
    // cout << endl;
    // cout << "the inserted part is: " << pair.key << " " << pair.value << endl;
    // cout << "insertion location is: " << first_larger_idx << endl;
    // cout << subroot->is_leaf << endl;
    /* TODO Your code goes here! */
    // first, if the subroot is the leaf node
    if (subroot->is_leaf){// && !(subroot->elements[first_larger_idx] == pair)){
      ...
      // cout << "after insertion, tree node is:" << endl;
      // cout << *subroot << endl;
      return;
    }
    // second, if the node is not a leaf
    else{// if (!subroot->is_leaf && !(subroot->elements[first_larger_idx] == pair)){
      insert(subroot->children[first_larger_idx], pair);
      // after the insertion, we need to check whether there is an overflow of the node
      // cout << "before split, subroot is:" << endl;
      // cout << *subroot << endl;
      // cout << "before split, child left is:" << endl;
      // cout << *(subroot->children[first_larger_idx]) << endl;
      // cout << "before split, child right is:" << endl;
      // cout << *(subroot->children[first_larger_idx+1]) << endl;
      // cout << endl;
      if (subroot->children[first_larger_idx]->elements.size() >= order){
        split_child(subroot, first_larger_idx);
        // cout << "after split, subroot is:" << endl;
        // cout << *subroot << endl;
        // cout << "after split, child left is:" << endl;
        // cout << *(subroot->children[first_larger_idx]) << endl;
        // cout << "after split, child right is:" << endl;
        // cout << *(subroot->children[first_larger_idx+1]) << endl;
        // cout << endl;
      }
    }
