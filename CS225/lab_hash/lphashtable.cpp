...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    // first, we need to find the location of the insert element
    size_t idx = hash(key, size);
    // make the pair pointer
    std::pair<K, V>* p = new std::pair<K, V> (key, value);
    // second, we need to check whether the location idx is occupied
    while (table[idx] != NULL) idx = (idx+1) % size;
    table[idx] = p;
    should_probe[idx] = true;
    // at last, we need to check whether we need to resize the table
    ++elems;
    if (shouldResize())
        resizeTable();
}
