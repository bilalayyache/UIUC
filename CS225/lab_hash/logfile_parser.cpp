...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         * vector as well.
         */
        string info = ll.customer + ll.url;
        // first, update the pageVisitedTable
        if (!pageVisitedTable.keyExists(ll.url))
          pageVisitedTable.insert(ll.url, true);
        // second, update the whenVisitedTable
        if (!whenVisitedTable.keyExists(info))
          whenVisitedTable.insert(info, ll.date);
        else if (whenVisitedTable[info] < ll.date){
          whenVisitedTable[info] = ll.date;
        }
    }
    infile.close();
    // at last, update the uniqueURLs
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
     ...
     */
    string info = customer + url;
    return whenVisitedTable.keyExists(info);
                                  ...
                                  const string& url) const
{
    ...
    if (hasVisited(customer, url))
      return whenVisitedTable.find(info);
    else
