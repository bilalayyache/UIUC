 ...
 */
SkipList::SkipList() {
    // zhi zhang a zhe ge cuo wu
    // TMD zhe ge dai ma shi na ge shabi xie de
    // modify here
    head_ = new SkipNode();
    tail_ = new SkipNode();
...
void SkipList::insert(int key, HSLAPixel value) {
    ...
    if (temp) {
        temp->value = value;
        // modify here since we do not want to modify the length of the list
        return;
    }
    ...
    // cout << "height of head is " << head_->nodePointers.size() << endl;
    // cout << "height of tail is " << tail_->nodePointers.size() << endl;
    ...
    //     }
    // }
    // int maxLevel = max(newNodeLevel, this->listHeight_);
    // int minLevel = min(newNodeLevel, this->listHeight_);
    int maxLevel = newNodeLevel;
    int minLevel = min(newNodeLevel, int(forward->nodePointers.size()));
    // cout << "maxLevel, minLevel " << maxLevel << " " << minLevel << endl;
    while (forwardLevel < maxLevel){
      if (forwardLevel < minLevel){
        forward->nodePointers[forwardLevel].prev = newNode;
        newNode->nodePointers[forwardLevel].next = forward;
        forwardLevel++;
      }
      ...
      // | | --> | |
      // new   forward
      else if (forwardLevel >= newNodeLevel){
        break;
      }
      // case when:
      // | | ---------->
      ...
      // | | --> | |
      // new   forward
      // forward cannot be tail in this case since the height of tail is larger or equal to newNodeLevel
      else{
        while (forward->nodePointers.size()-1 < unsigned(forwardLevel)){
          forward = forward->nodePointers[forwardLevel-1].next;
        }
        forward->nodePointers[forwardLevel].prev = newNode;
        newNode->nodePointers[forwardLevel].next = forward;
        forwardLevel++;
    ...
    // backwards case
    minLevel = min(newNodeLevel, int(prev->nodePointers.size()));
    while (backwardLevel < maxLevel){
      if (backwardLevel < minLevel){
        prev->nodePointers[backwardLevel].next = newNode;
        newNode->nodePointers[backwardLevel].prev = prev;
        // if (key == 16){
        //   cout << "16:" << endl;
        //   cout << newNodeLevel << endl;
        //   cout << listHeight_ << endl;
        //   cout << backwardLevel << endl;
        //   cout << newNode << " " << newNode->nodePointers[0].prev << endl;
        // }
        backwardLevel++;
      }
      // case when:
      ...
      // | | <-- | |
      // prev    new
      else if (backwardLevel >= newNodeLevel){
        break;
      }
      // case when:
      // <------ | |
      ...
      // | | <-- | |
      // prev    new
      // prev cannot be head in this case since the height of tail is larger or equal to newNodeLevel
      else{
        // cout << "backwardLevel is " << backwardLevel << endl;
        // cout << "newNodeLevel is " << newNodeLevel << endl;
        // cout << "prev key is " << prev << " " << prev->key << endl;
        while (prev->nodePointers.size()-1 < unsigned(backwardLevel)){
          prev = prev->nodePointers[backwardLevel-1].prev;
          // cout << "prev is " << prev << " " << prev->key << endl;
        }
        prev->nodePointers[backwardLevel].next = newNode;
        newNode->nodePointers[backwardLevel].prev = prev;
        backwardLevel++;
    ...
    // 
    // traverse = head_;
    // while (traverse != tail_){
    //   cout << traverse << " " << traverse->key << endl;
    //   traverse = traverse->nodePointers[0].next;
    // }
    // cout << endl;
    // traverse = tail_;
    // while (traverse != head_){
    //   cout << traverse << " " << traverse->key << endl;
    //   traverse = traverse->nodePointers[0].prev;
    // }
    // cout << endl;
}
