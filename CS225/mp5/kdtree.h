...
class KDTree
{
  ...
  private:
     ...
     */
    // helper function 1
    // find the median point from the input vector
    // and partition the input vector to two other vector
    int partition(vector<Point<Dim>>& v, int low, int high, int levels);
    void quicksort(vector<Point<Dim>>& input, int low, int high, int levels);

    // helper function to constructor
    // void construct(KDTreeNode*& root, vector<Point<Dim>>& input, int levels);
    typename KDTree<Dim>::KDTreeNode* construct(vector<Point<Dim>>& input, int levels);

    // helper function to copy the whole tree
    typename KDTree<Dim>::KDTreeNode* copy(KDTreeNode *root);
    // helper function to delete the tree
    void clear(KDTreeNode *root);

    // helper function to find the leaf of the tree that contains the query
    void findNN(const KDTreeNode* root, Point<Dim>& nearest, const Point<Dim>& query, double* dis, int levels) const;
    void find(const KDTreeNode* root, Point<Dim>& nearest, const Point<Dim>& query, double* dis, int levels) const;
