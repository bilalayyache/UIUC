...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  Animation animation;
  /** @todo [Part 2] */
  size_t count = traversal_.size();
  for (size_t i = 0; i < count; i++){
    // count how many pixels are modified
    ImageTraversal* tra = traversal_[i];
    ColorPicker* col = colorPicker_[i];
    unsigned count = 0;
    for (const Point & point : (*tra)){
      if (count%frameInterval == 0){
        animation.addFrame(png_);
      }
      unsigned x_ = point.x; unsigned y_ = point.y;
      HSLAPixel& pixel = png_.getPixel(x_, y_);
      pixel = col->getColor(x_, y_);
      count++;
    }
    // add the final frame
    animation.addFrame(png_);
