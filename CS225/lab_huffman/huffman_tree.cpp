...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    // first step, push the treenode onto singleQueue
    ...
    // second step, when sum of two queue size is greater than 1,
    // we need to create internal TreeNode
    while (singleQueue.size() + mergeQueue.size() > 1){
      // cout << singleQueue.size() + mergeQueue.size() << endl;
      // find two TreeNode of the smallest frequency from two queues
      TreeNode *temp1, *temp2;
      temp1 = removeSmallest(singleQueue, mergeQueue);
      temp2 = removeSmallest(singleQueue, mergeQueue);
      // cout << "temp1 is " << temp1->freq.getCharacter() << ": " << temp1->freq.getFrequency() << endl;
      // cout << "temp2 is " << temp2->freq.getCharacter() << ": " << temp2->freq.getFrequency() << endl;
      // after finding two smallest node, we need to construct the internode of the tree
      TreeNode* internal = new TreeNode(temp1->freq.getFrequency() + temp2->freq.getFrequency());
      internal->left = temp1;
      internal->right = temp2;
      // finally, push back internal onto the mergeQueue
      mergeQueue.push(internal);
    }
    // at last, determine the root node
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
      // first, find the character in the tree
      ...
      // after finding the character, put the char into stream
      char retval = current->freq.getCharacter();
      ss << retval;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()){
      if (bfile.getNextBit() == 1){
        char leaf = bfile.getNextByte();
        node = new TreeNode(Frequency(leaf, 0));
        return node;
      }
      else{
        node = new TreeNode(Frequency(0));
